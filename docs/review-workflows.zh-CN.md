...
核心只确认文件存在，不认证签名、不审阅证据正文、不证明模型执行、不读取当前工作区，也不能证明记录时间对应真实远端读取。替换证据文件内容不会被候选摘要检测到。用于实际放行时，适配器必须核验证据完整性，在动作前重新比对真实代码树和远端引用，并独立执行授权检查。使用真实时钟重新校验，API 的时间注入只用于确定性测试。可选的 [Jev 适配器](jev.zh-CN.md)可通过显式请求生成模型证据。校验核心保持离线，不包含密钥管理、企业角色、消息或部署连接器。
