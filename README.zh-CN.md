...
初始化不会覆盖已有文件。生成内容包括 `deliveryguard.config.json`、可复用的 `AGENTS.md` Harness 合同、`.deliveryguard/`、`openspec/changes/`，以及覆盖完整交付流程的 20 个供应商无关 Codex Skills，其中包括请求链路排查、录屏视觉诊断与经授权的真机测试指引。
